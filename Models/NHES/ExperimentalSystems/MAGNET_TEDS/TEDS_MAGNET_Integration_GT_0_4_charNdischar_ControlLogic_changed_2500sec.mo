...
model TEDS_MAGNET_Integration_GT_0_4_charNdischar_ControlLogic_changed_2500sec
...
public
  ...
  Modelica.Fluid.Sources.Boundary_pT boundary4(
    ...
    T=323.15,
    nPorts=1)
    annotation (Placement(transformation(extent={{620,-144},{600,-164}})));
  ...
  Modelica.Fluid.Sources.MassFlowSource_T Chiller_Mass_Flow1(
    ...
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=180,
        origin={610,-202})));
  Modelica.Blocks.Sources.Constant const2(k=0.01)                          annotation (
     Placement(transformation(
        extent={{5,-5},{-5,5}},
        rotation=0,
        origin={595,-183})));
...
equation
  ...
  connect(TEDS_HX.port_b_tube, boundary4.ports[1]) annotation (Line(points={{
          566,-164},{566,-154},{600,-154}}, color={0,127,255}));
  connect(T_chiller_before.port_b, TEDS_HX.port_a_shell) annotation (Line(
        points={{562,-126},{560.48,-126},{560.48,-164}}, color={0,127,255}));
  connect(TEDS_HX.port_b_shell, T_chiller_after.port_a)
    annotation (Line(points={{560.48,-188},{561,-244}}, color={0,127,255}));
  connect(Chiller_Mass_Flow1.ports[1], valve1.port_b)
    annotation (Line(points={{600,-202},{588,-202}}, color={0,127,255}));
  connect(valve1.port_a, TEDS_HX.port_a_tube) annotation (Line(points={{576,
          -202},{566,-202},{566,-188}}, color={0,127,255}));
  connect(valve1.opening, const2.y) annotation (Line(points={{582,-197.2},{582,
          -183},{589.5,-183}}, color={0,0,127}));
...
end TEDS_MAGNET_Integration_GT_0_4_charNdischar_ControlLogic_changed_2500sec;
