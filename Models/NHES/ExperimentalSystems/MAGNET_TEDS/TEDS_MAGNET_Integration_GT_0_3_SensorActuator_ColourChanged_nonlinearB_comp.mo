...
model
  TEDS_MAGNET_Integration_GT_0_3_SensorActuator_ColourChanged_nonlinearB_comp
...
public
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort sensor_hx_cw(
    ...
    annotation (Placement(transformation(extent={{124,-250},{144,-230}})));
public
...
public
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort sensor_cw_hx(
    ...
    redeclare function iconUnit2 =
        TRANSFORM.Units.Conversions.Functions.Temperature_K.to_degC)
    annotation (Placement(transformation(extent={{0,-248},{20,-228}})));
public
...
public
  ...
  Systems.BalanceOfPlant.Turbine.NonLinear_Break nonLinear_Break(redeclare
      package Medium = Medium_cw)
    annotation (Placement(transformation(extent={{30,-248},{50,-228}})));
  Systems.BalanceOfPlant.Turbine.NonLinear_Break nonLinear_Break3(redeclare
      package Medium = Medium_cw)
    annotation (Placement(transformation(extent={{136,-286},{122,-270}})));
equation
  connect(sensor_hx_cw.port_b,boundary. ports[1])
    annotation (Line(points={{144,-240},{152,-240}},
                                               color={0,127,255}));
  ...
  connect(mflow_cw.port_a, boundary1.ports[1])
    annotation (Line(points={{-26,-238},{-34,-238}}, color={0,127,255}));
  connect(mflow_cw.port_b, sensor_cw_hx.port_a)
    annotation (Line(points={{-6,-238},{0,-238}},  color={0,127,255}));
  ...
  connect(sensorSubBus.heater_BOP_massflow, BOP_Mass_flow.m_flow) annotation (
      Line(
      points={{303,-22},{574,-22},{574,-44},{508,-44},{508,-48.4},{440,-48.4}},
      color={238,46,47},
      pattern=LinePattern.Dash,
      thickness=0.5));
  ...
  connect(MAGNET_TEDS_simpleHX1.port_b1, valve_TEDS_rp.port_a) annotation (Line(
        points={{246,-78},{0,-78},{0,-82},{4.44089e-16,-82},{4.44089e-16,-84}},
        color={0,127,255}));
  ...
  connect(hx.port_a2, sensor_hx_cw.port_a) annotation (Line(points={{82,-258},{
          112,-258},{112,-240},{124,-240}}, color={0,127,255}));
  connect(sensor_hx_co.port_b, valve_ps.port_b)
    annotation (Line(points={{30,-278},{-32,-278}}, color={0,127,255}));
  connect(sensor_cw_hx.port_b, nonLinear_Break.port_a)
    annotation (Line(points={{20,-238},{30,-238}}, color={0,127,255}));
  connect(nonLinear_Break.port_b, hx.port_b2) annotation (Line(points={{50,-238},
          {56,-238},{56,-258},{62,-258}}, color={0,127,255}));
  connect(pipe_ins_rp_hx.port_b, nonLinear_Break3.port_a)
    annotation (Line(points={{142,-278},{136,-278}}, color={0,127,255}));
  connect(nonLinear_Break3.port_b, sensor_rp_hx_2.port_a)
    annotation (Line(points={{122,-278},{116,-278}}, color={0,127,255}));
...
end TEDS_MAGNET_Integration_GT_0_3_SensorActuator_ColourChanged_nonlinearB_comp;
