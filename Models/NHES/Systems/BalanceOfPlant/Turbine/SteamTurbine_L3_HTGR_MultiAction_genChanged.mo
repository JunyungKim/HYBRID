...
model SteamTurbine_L3_HTGR_MultiAction_genChanged
  ...
  TRANSFORM.Fluid.Valves.ValveLinear TCV(
    ...
    m_flow_nominal=50) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-4,40})));
  Modelica.Blocks.Sources.RealExpression Electrical_Power(y=generator.Q_elec)
  ...
  TRANSFORM.Electrical.Interfaces.ElectricalPowerPort_Flow port_e
    annotation (Placement(transformation(extent={{130,-26},{150,-6}}),
        iconTransformation(extent={{130,-26},{150,-6}})));
  ...
  Electrical.PowerSensor                   sensorW
    annotation (Placement(transformation(extent={{212,-26},{192,-6}})));
  Modelica.Blocks.Continuous.Integrator ElectricityCumulative(use_reset=false,
      use_set=false)
...
equation
  ...
  connect(actuatorBus.openingLPTv, LPT1_Bypass.opening) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(generator.shaft, LPT2.shaft_b) annotation (Line(points={{238,34},{218,
          34}},                 color={0,0,0}));
  ...
  connect(generator.port, sensorW.port_a)
    annotation (Line(points={{238,14},{238,-16},{212,-16}}, color={255,0,0}));
  connect(sensorW.port_b, port_e)
    annotation (Line(points={{192,-16.2},{166,-16.2},{166,-16},{140,-16}},
                                                   color={255,0,0}));
...
end SteamTurbine_L3_HTGR_MultiAction_genChanged;
