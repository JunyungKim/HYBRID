...
model SteamTurbine_L3_HTGR_v2_8_ISeeHope
  extends BaseClasses.Partial_SubSystem(
    redeclare replaceable
      ControlSystems.CS_threeStagedTurbine_HTGR_v3
      CS,
    redeclare replaceable ControlSystems.ED_Dummy ED,
    redeclare Data.HTGR_3_BOP   data(
      valve_TBV_dp_nominal=500000,
      valve_TCV_dp_nominal=500000,
      valve_LPT1_Bypass_dp_nominal=500000,
      valve_LPT2_Bypass_mflow=30,
      valve_LPT2_Bypass_dp_nominal=500000,
      BypassFeedHeater_NTU=10));
  Real time_altered;
  ...
  Fluid.Vessels.IdealCondenser Condenser(
    ...
    annotation (Placement(transformation(extent={{244,-78},{232,-66}})));
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance R_InternalBypass(R=5e5,
      redeclare package Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(
        extent={{4,-4},{-4,4}},
        rotation=270,
...
equation
  ...
  connect(Condenser.port_b, FWP.port_a) annotation (Line(
      points={{238,-78},{238,-82},{160,-82}},
      color={0,127,255},
      thickness=0.5));
  connect(FWP.port_b, volume1.port_a) annotation (Line(
      points={{140,-82},{122,-82},{122,-58},{84,-58}},
      color={0,127,255},
      thickness=0.5));
  ...
  connect(LPT2_Bypass.port_b, BypassFeedwaterHeater.Shell_in) annotation (Line(
        points={{178,6},{178,-16},{4,-16},{4,-52},{8,-52}}, color={0,127,255}));
  connect(BypassFeedwaterHeater.Shell_out, LPT2_Bypass1.port_a)
    annotation (Line(points={{28,-52},{90,-52},{90,-36},{152,-36}},
                                                  color={0,127,255}));
  connect(realExpression.y, LPT2_Bypass1.opening)
    annotation (Line(points={{111,-28},{162,-28}},           color={0,0,127}));
  connect(LPT2_Bypass1.port_b, volume2.port_b[1]) annotation (Line(points={{172,
          -36},{242,-36},{242,-38},{242,-40.4},{242.3,-40.4}}, color={0,127,255}));
  connect(volume2.port_a[1], R_InternalBypass.port_b)
    annotation (Line(points={{242,-47.6},{242,-53.2}}, color={0,127,255}));
  connect(R_InternalBypass.port_a, Condenser.port_a) annotation (Line(points={{
          242,-58.8},{242,-66},{242.2,-66}}, color={0,127,255}));
  connect(volume2.port_b[2], LPT2.portLP) annotation (Line(points={{241.7,-40.4},
          {242,-40.4},{242,40},{218,40}}, color={0,127,255}));
...
end SteamTurbine_L3_HTGR_v2_8_ISeeHope;
