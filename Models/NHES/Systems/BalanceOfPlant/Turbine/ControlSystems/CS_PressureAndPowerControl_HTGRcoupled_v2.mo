...
model CS_PressureAndPowerControl_HTGRcoupled_v2
  ...
  input SI.Temperature temp_steamSetpoint "SG outlet steam temperature setpoint" annotation(Dialog(group="Inputs"));
  ...
  Modelica.Blocks.Sources.RealExpression W_totalSetpoint_BOP(y=
        temp_steamSetpoint)
...
equation
  ...
  connect(sensorBus.p_inlet_steamTurbine, PID_TCV_opening.u_m) annotation (Line(
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(sensorBus.Steam_Temperature, PID_BV_opening.u_m) annotation (Line(
      points={{-30,-100},{-30,-58},{-50,-58},{-50,-50}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
...
end CS_PressureAndPowerControl_HTGRcoupled_v2;
