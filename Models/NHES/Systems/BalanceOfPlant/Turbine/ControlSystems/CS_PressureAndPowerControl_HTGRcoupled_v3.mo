...
model CS_PressureAndPowerControl_HTGRcoupled_v3
  ...
  input SI.Temperature temp_steamSetpoint "steam temperature setpoint" annotation(Dialog(group="Inputs"));
  ...
  Modelica.Blocks.Sources.RealExpression temp_steamSetpoint_BOP(y=
        temp_steamSetpoint)
...
equation
  ...
  connect(sensorBus.W_total, PID_BV_opening.u_m) annotation (Line(
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(temp_steamSetpoint_BOP.y, PID_TCV_opening.u_s)
    annotation (Line(points={{-69,6},{-62,6}}, color={0,0,127}));
  connect(sensorBus.Steam_Temperature, PID_TCV_opening.u_m) annotation (Line(
      points={{-30,-100},{-30,-16},{-50,-16},{-50,-6}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
...
end CS_PressureAndPowerControl_HTGRcoupled_v3;
